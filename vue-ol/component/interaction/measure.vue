...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            formatLength(line) {   //测量长度输出
                const length = Math.round(line.getLength() * 100) / 100; //直接得到线的长度
                let output;
                if (length > 100) {
            ...
            formatArea(polygon) {   //测量面积输出
                const area = polygon.getArea(); //直接获取多边形的面积
                let output;
                if (area > 10000) {
                ...
                this.draw.on("drawstart", function(evt){
                    ...
                    vm.listenerChange = vm.sketch.getGeometry().on('change', function (evt) {
                        ...
                        if (geom instanceof Polygon) {
                            resultValue = vm.formatArea(geom);
                            output += "<div>面积：" + resultValue + "</div><div>单击继续绘制，双击结束</div>"; //面积值
                        } else if (geom instanceof LineString) {
                            resultValue = vm.formatLength(geom);
            ...
            endDraw(map){
                ...
                vm.draw.on("drawend", function (evt) {
                    ...
                    let output = "",
                        coords, len;
                    if (geom instanceof Polygon) {
                        output += "<div>面积：" + vm.formatArea(geom) + "</div>"; //面积值
                        ...
                        len = coords.length;
                        output = "<div>总长：" + vm.formatLength(geom) + "</div>"; //长度值
